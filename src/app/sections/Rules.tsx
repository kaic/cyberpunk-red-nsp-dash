      ...
      <h1 className="text-4xl font-bold mb-6 text-green-400 flex items-center border-b border-green-900 pb-2">
        ...
        <GlitchText>Rules</GlitchText>
          ...
          <div className="mb-6">
              ...
              <div className={`${menuOpen ? 'block' : 'hidden md:grid'} md:grid-cols-3 lg:grid-cols-4 gap-2 bg-black/90 p-3 rounded-lg border border-green-900/50`}>
                  ...
                  className={`px-3 py-2 rounded-lg flex items-center mb-1 ${activeSection === section.id
              ...
              <div className="relative z-10">
                      ...
                      >
                          <h2 className="text-2xl font-bold text-green-400 flex items-center">
                              {'icon' in item && item.icon && <span className="mr-3 text-2xl">{item.icon}</span>}
                      ...
                      {/* Conteúdo colapsável */}
                      <div
                          className={isExpanded(`${currentSection.id}-${index}`) ? 'p-4' : 'hidden'}
                      >
                          ...
                          <div key={stateIndex} className="bg-black border border-green-800 rounded-lg p-4">
                              <h4 className="font-bold text-green-400 mb-2 text-lg border-b border-green-900/50 pb-2">{state.state}</h4>
                              <div className="space-y-2">
                                  <p className="text-gray-400"><span className="text-green-500 font-bold">Condição:</span> {state.condition}</p>
                                  <p className="text-gray-400"><span className="text-green-500 font-bold">Efeitos:</span> {state.effects}</p>
                                  <p className="text-gray-400"><span className="text-green-500 font-bold">Cura:</span> {state.healing}</p>
                              </div>
                          </div>
                      ))}
                              </div>
...
const RenderSubsection = ({ subsection, sectionId }: { subsection: any; sectionId: string; itemIndex: number; subIndex: number }) => {
  return (
      <div className="bg-black border border-green-900 rounded-lg overflow-hidden">
          ...
          <div className="p-4">
              ...
              {subsection.actions && (
                ...
                <div key={i} className="bg-green-900/10 p-3 rounded-lg border-l-2 border-green-700">
                    <span className="text-green-400 font-bold block mb-1">{action.name}</span>
              ...
              {subsection.parts && (
                ...
                <div key={i} className="bg-black/50 rounded-lg p-4 border-l-2 border-green-700">
                    ...
                    {part.example && (
                        <div className="bg-green-900/10 border border-green-800/50 rounded-lg p-3 mt-2">
                            <p className="text-gray-400 mb-2">{part.example.scenario}</p>
                            <p className="text-green-400 font-mono">{part.example.result}</p>
                            {part.example.note && (
                                <p className="text-gray-500 text-sm italic mt-1">{part.example.note}</p>
                            )}
                        </div>
                    )}
                </div>
              ...
              {subsection.example && (
                  <div className="mb-4 bg-green-900/10 border border-green-800 rounded-lg p-4">
                    ...
                    <li key={i} className="flex items-start text-sm">
                      ...
                      <p className="text-green-400 font-mono bg-black/30 p-2 rounded">{subsection.example.result}</p>

                      {subsection.example.conclusion && (
                          <p className="text-green-400 font-mono mt-2">{subsection.example.conclusion}</p>
                      )}

                      {subsection.example.note && (
                          <p className="text-gray-500 text-sm italic mt-2 border-t border-green-900/30 pt-2">{subsection.example.note}</p>
                      )}
                  </div>
              )}
              ...
              {/* Single note */}
              {subsection.note && (
                  <p className="text-gray-500 italic mb-4 bg-black/30 p-3 rounded-lg border-l-2 border-yellow-700">{subsection.note}</p>
              )}
              ...
              {subsection.factors && (
                  ...
                  <li key={i} className="bg-black/30 p-3 rounded-lg border-l-2 border-green-700">
                      <span className="text-green-400 font-bold block mb-1">{factor.name}</span>
              ...
              {subsection.gaining && (
                  <div className="mb-4 space-y-3">
                      <div className="bg-green-900/10 p-3 rounded-lg border-l-2 border-green-600">
                          <h4 className="font-bold text-green-400 mb-2">Ganha reputação:</h4>
                    ...
                    <li key={i} className="flex items-start">
                      ...
                      {subsection.losing && (
                          <div className="bg-red-900/10 p-3 rounded-lg border-l-2 border-red-700">
                              <h4 className="font-bold text-red-400 mb-2">Perde reputação:</h4>
    ...
    if (subsection.howToBuy) {
        ...
        <div className="space-y-4">
            {Object.entries(subsection.howToBuy).map(([key, value]) => {
                ...
                return (
              <div key={key} className="bg-black/40 rounded-lg overflow-hidden border border-green-900">
                  <div className="bg-green-900/30 p-3 border-b border-green-800">
                      <h4 className="font-bold text-green-400">Passo {key.slice(-1)}: {step.title}</h4>
                  </div>
                              ...
                              <table className="w-full text-sm">
                                  <thead>
                                      <tr className="border-b border-green-800/50 bg-green-900/20">
                                          <th className="text-left py-2 px-3 text-green-400">Raridade</th>
                                          <th className="text-center py-2 px-3 text-green-400">DV</th>
                                      </tr>
                                  </thead>
                                  <tbody>
                                      {step.difficulties.map((diff, i) => (
                                          <tr key={i} className={`border-b border-green-900/20 ${i % 2 === 0 ? 'bg-black' : 'bg-green-900/10'}`}>
                                <td className="text-left py-2 px-3 text-gray-400">{diff.rarity}</td>
                                <td className="text-center py-2 px-3 text-gray-400">{diff.dv}</td>
                          </tr>
                        ))}
                                  </tbody>
                              </table>
                          ...
                          <div className="mb-3 overflow-x-auto rounded-lg border border-green-800/50">
                              <table className="w-full text-sm">
                                  <thead>
                                      <tr className="border-b border-green-800/50 bg-green-900/20">
                                          <th className="text-left py-2 px-3 text-green-400">Raridade</th>
                                          <th className="text-center py-2 px-3 text-green-400">Tempo</th>
                                      </tr>
                                  </thead>
                                  <tbody>
                                      {step.deliveryTimes.map((time, i) => (
                                          <tr key={i} className={`border-b border-green-900/20 ${i % 2 === 0 ? 'bg-black' : 'bg-green-900/10'}`}>
                                <td className="text-left py-2 px-3 text-gray-400">{time.rarity}</td>
                                <td className="text-center py-2 px-3 text-gray-400">{time.time}</td>
                          </tr>
                        ))}
                                  </tbody>
                              </table>
                      ...
                      {step.complications && (
                          <div className="mb-3 overflow-x-auto rounded-lg border border-green-800/50">
                              <table className="w-full text-sm">
                                  <thead>
                                      <tr className="border-b border-green-800/50 bg-green-900/20">
                                          <th className="text-left py-2 px-3 text-green-400">Dado</th>
                                          <th className="text-left py-2 px-3 text-green-400">Evento</th>
                                      </tr>
                                  </thead>
                                  <tbody>
                                      {step.complications.map((comp, i) => (
                                          <tr key={i} className={`border-b border-green-900/20 ${i % 2 === 0 ? 'bg-black' : 'bg-green-900/10'}`}>
                                <td className="text-left py-2 px-3 text-gray-400">{comp.roll}</td>
                                <td className="text-left py-2 px-3 text-gray-400">{comp.result}</td>
                          </tr>
                        ))}
                                  </tbody>
                              </table>
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-green-800 bg-green-900/20">
                        ...
                        <th className="text-center py-3 px-3 text-green-400">Preço (€$)</th>
                ...
                <tbody>
                    ...
                    <td className="text-left py-2 px-3 text-gray-300 font-medium">{item.item}</td>
                    <td className="text-center py-2 px-3 text-gray-400">{item.rarity}</td>
                    <td className="text-center py-2 px-3 text-gray-400 font-mono">{item.price}</td>
                    <td className="text-left py-2 px-3 text-gray-400">{item.notes}</td>
                </tr>
            ))}
                  </tbody>
              </table>
    ...
    if (subsection.costOfLiving) {
        ...
        <div className="overflow-x-auto rounded-lg border border-green-800/50">
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-green-800 bg-green-900/20">
                        <th className="text-left py-3 px-3 text-green-400">Estilo de Vida</th>
                        <th className="text-center py-3 px-3 text-green-400">Custo mensal</th>
                ...
                <tbody>
                    {subsection.costOfLiving.map((style, i) => (
                        <tr key={i} className={`border-b border-green-900/30 ${i % 2 === 0 ? 'bg-black' : 'bg-green-900/10'}`}>
                    <td className="text-left py-2 px-3 text-gray-300 font-medium">{style.style}</td>
                    <td className="text-center py-2 px-3 text-green-400 font-mono">{style.cost}</td>
                    <td className="text-left py-2 px-3 text-gray-400">{style.description}</td>
                </tr>
            ))}
                  </tbody>
              </table>
